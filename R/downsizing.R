...
downsizeMatrix <- function (data, level = 1.1e+07, HQ_reads=NULL, repetitions = 1, silent = FALSE)
{
  ...
  if (is.null(HQ_reads))
  {
    for (ind in 1:ncol(data)) 
    {
      if (!silent) 
      {
        print(paste(ind, "Sample", colnames(data)[ind], "with", sum(data[, ind]), "reads and", sum(data[, ind] != 0), "genes"))
      }
